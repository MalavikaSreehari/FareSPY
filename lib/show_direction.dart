...
import 'package:farespy/live_loc.dart';
...
class _ShowDirectionState extends State<ShowDirection> {
  ...
  _getPolyline() async {
    PolylineResult result = await polylinePoints.getRouteBetweenCoordinates(
      placesKey,
      PointLatLng(widget.startPoint!.geometry!.location!.lat!,
          widget.startPoint!.geometry!.location!.lng!),
      PointLatLng(widget.endPoint!.geometry!.location!.lat!,
          widget.endPoint!.geometry!.location!.lng!),
  ...
  Widget build(BuildContext context) {
    Set<Marker> _markers = {
      Marker(
          ...
          position: LatLng(widget.startPoint!.geometry!.location!.lat!,
              widget.startPoint!.geometry!.location!.lng!)),
      Marker(
          markerId: MarkerId('end'),
          position: LatLng(widget.endPoint!.geometry!.location!.lat!,
              widget.endPoint!.geometry!.location!.lng!))
    };
    return Scaffold(
      body: SafeArea(
            ...
            child: ElevatedButton(
              onPressed: () {
                ...
                Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (context) => FareDisplay(distance: distance)),
                );
              },
              style:
                  ElevatedButton.styleFrom(backgroundColor: Color(0xff93C561)),
              child: Text(
                "Get Fare",
                style: TextStyle(
                  color: Colors.white,
